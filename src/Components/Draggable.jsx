...
export const Drag = () => 
{
    ...
    const onDragEnd = result => {
        ...
        if (srcDroppableId === destDroppableId) {
            const list = srcDroppableId === 'droppable' ? [...gameState.items] : [...gameState.selected];
            ...
            const sourceList = srcDroppableId === 'droppable' ? [...gameState.items] : [...gameState.selected];
            const destList = destDroppableId === 'droppable' ? [...gameState.items] : [...gameState.selected];
                            ...
                            {(provided, snapshot) => (
                                <div ref={provided.innerRef} style={getListStyle(snapshot.isDraggingOver)}>
                                    {gameState.items.map((item, index) => (
                                        <Draggable key={item['PLAYER_ID']} draggableId={item['PLAYER_ID']} index={index}>
                                            {(provided, snapshot) => (
                                                <div
                                                    ...
                                                    style={getItemStyle(snapshot.isDragging, provided.draggableProps.style)}>
                                                        <Card name={item['PLAYER_NAME']} id={item['PLAYER_ID']}/>
                                                </div>
                                            )}
                                        </Draggable>
                                    ))}
